...
public class DateTimePicker extends WheelPicker {
    ...
    private String yearLabel = isChinese? "年":"year", monthLabel = isChinese? "月":"month", dayLabel = isChinese? "日":"day";
    //设置显示位置  0 显示列表中间   默认 1显示在内容旁边
    public int yearLabelsite = 1, monthLabelsite = 1, dayLabelsite = 1;
    private String hourLabel = isChinese? "时":"hour", minuteLabel = isChinese? "分":"minute";
    //设置显示位置  0 显示列表中间   默认 1显示在内容旁边
    public int hourLabelsite = 1, minuteLabelsite = 1;
...
//        this.yearLabelsite = 1;
//        this.monthLabelsite = 1;
//        this.dayLabelsite = 1;
        setLabel(yearLabel,monthLabel,dayLabel,hourLabel,minuteLabel,1,1,1,1,1);
    }

    /**
     * 设置年月日时分的显示单位
     */
    public void setLabel(String yearLabel, String monthLabel, String dayLabel, String hourLabel, String minuteLabel,int yearLabelsite,int monthLabelsite,int dayLabelsite,int hourLabelsite,int minuteLabelsite) {
        this.yearLabel = yearLabel;
        this.monthLabel = monthLabel;
        this.dayLabel = dayLabel;
        this.hourLabel = hourLabel;
        this.minuteLabel = minuteLabel;
        this.yearLabelsite = yearLabelsite;
        this.monthLabelsite = monthLabelsite;
        this.dayLabelsite = dayLabelsite;
        this.hourLabelsite = hourLabelsite;
        this.minuteLabelsite = minuteLabelsite;
    }
...
//        wheelViewParams.gravity = Gravity.CENTER;
        if(wheelModeEnable){
            ...
            if (dateMode == YEAR_MONTH_DAY || dateMode == YEAR_MONTH) {
                ...
                if (!TextUtils.isEmpty(yearLabel)&&yearLabelsite==1) {
                    yearView.setLabel(yearLabel);
                }
//                yearView.isCenterLabel(true);
//                yearView.setGravity(Gravity.CENTER);
                ...
                if (!TextUtils.isEmpty(yearLabel)&&yearLabelsite==0){
            ...
            if (dateMode != NONE) {
                ...
                if (!TextUtils.isEmpty(monthLabel)&&monthLabelsite==1) {
...
//                        }
                ...
                if (!TextUtils.isEmpty(monthLabel)&&monthLabelsite==0){
                    TextView labelView = new TextView(activity);
                    labelView.setLayoutParams(labelViewParams);
                    labelView.setTextColor(textColorFocus);
                    labelView.setTextSize(textSize);
            ...
            if (dateMode == YEAR_MONTH_DAY || dateMode == MONTH_DAY) {
                ...
                if (!TextUtils.isEmpty(dayLabel)&&dayLabelsite==1) {
                ...
                if (!TextUtils.isEmpty(dayLabel)&&dayLabelsite==0){
                    TextView labelView = new TextView(activity);
                    labelView.setLayoutParams(labelViewParams);
                    labelView.setTextColor(textColorFocus);
                    labelView.setTextSize(textSize);
